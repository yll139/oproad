...
Implementation reuses existing project outputs by default. For example, if
`oproad synth` already produced `results/<platform>/<design>/base/1_synth.v`
and the RTL/SDC/config dependencies have not changed, `oproad implement` lets
the Makefile keep that synthesis result instead of running Yosys again. Set
`OPROAD_REUSE_OUTPUTS=0` before running a task to force the container workspace
to start without cached outputs.
